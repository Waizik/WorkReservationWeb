...
The booking endpoint is additionally rate limited per client IP (`x-forwarded-for`). The limit comes from the `RateLimit__ReservationsPerHour` app setting; the CD pipeline sets it from the GitHub variable `RATE_LIMIT_RESERVATIONS_PER_HOUR` (default 3 bookings per hour per IP). Requests over the limit get HTTP 429 before any captcha, database, or e-mail work happens. An empty value disables the limiter (local development, tests). The limiter wraps the BCL `System.Threading.RateLimiting` partitioned fixed-window limiter; counters are in-memory per Functions instance — they reset on restart and are not shared across scaled-out instances, which is an accepted trade-off for a free, dependency-less setup.

Finally, one customer e-mail can hold at most N upcoming (confirmed, not yet started) reservations. The limit comes from the `Booking__MaxOpenReservationsPerEmail` app setting; the CD pipeline sets it from the GitHub variable `MAX_OPEN_RESERVATIONS_PER_EMAIL` (default 3). Cancelled and past reservations do not count. An empty value disables the check (local development, tests).
